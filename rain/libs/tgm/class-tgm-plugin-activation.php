...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function __construct() {
			...
			$this->strings = array(
				'page_title'                      => __( '主题扩展', $this->domain ),
				'menu_title'                      => __( '扩展', $this->domain ),
				'installing'                      => __( '正在安装： %s', $this->domain ),
				'oops'                            => __( '发生错误，安装失败！', $this->domain ),
				'notice_can_install_required'     => _n_noop( '当前主题需要安装以下插件：%1$s。', '当前主题需要安装以下插件： %1$s。' ),
				'notice_can_install_recommended'  => _n_noop( '当前主题推荐安装以下插件：%1$s。', '当前主题推荐安装以下插件： %1$s。' ),
				'notice_cannot_install'           => _n_noop( '安装失败：%s 需要管理员权限，请联系管理员以获得帮助。', '安装失败：%s 需要管理员权限，请联系管理员以获得帮助。' ),
				'notice_can_activate_required'    => _n_noop( '以下主题必需插件未激活： %1$s。', '以下主题必需插件未激活： %1$s。' ),
				'notice_can_activate_recommended' => _n_noop( '以下主题推荐插件未激活： %1$s。', '以下主题推荐插件未激活： %1$s。' ),
				'notice_cannot_activate'          => _n_noop( '激活失败：%s 需要管理员权限，请联系管理员以获得帮助。', '激活失败：%s 需要管理员权限，请联系管理员以获得帮助。' ),
				'notice_ask_to_update'            => _n_noop( '以下插件检测到更新： %1$s。', '以下插件检测到更新： %1$s。' ),
				'notice_cannot_update'            => _n_noop( '更新失败：%s 需要管理员权限，请联系管理员以获得帮助。', '更新失败：%s 需要管理员权限，请联系管理员以获得帮助。' ),
				'install_link' 					  => _n_noop( '立即安装', '立即安装' ),
				'activate_link' 				  => _n_noop( '立即激活', '立即激活' ),
				'return'                          => __( '返回扩展', $this->domain ),
				'plugin_activated'                => __( '激活成功！', $this->domain ),
				'complete'                        => __( '操作完成！%1$s', $this->domain ),
			);
			...
			if ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-install' )] ) && 'install-plugin' == $_GET[sanitize_key( 'tgmpa-install' )] ) ) {
				...
				if ( $this->is_automatic ) {
					...
					if ( is_wp_error( $activate ) ) {
						...
						echo '<p><a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . __( '返回扩展', $this->domain ) . '</a></p>';
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . admin_url() . '" title="' . __( '返回仪表盘', $this->domain ) . '">' . __( '返回仪表盘', $this->domain ) . '</a>' ) . '</p>';
			...
			elseif ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-activate' )] ) && 'activate-plugin' == $_GET[sanitize_key( 'tgmpa-activate' )] ) ) {
				...
				if ( is_wp_error( $activate ) ) {
					...
					if ( ! isset( $_POST[sanitize_key( 'action' )] ) ) {
						$msg = sprintf( __( '以下插件激活成功： %s。', $this->domain ), '<strong>' . $plugin['name'] . '</strong>' );
				...
				if ( ! empty( $message ) ) {
					...
					$action_links = apply_filters(
						...
						array(
							...
							'dismiss'  => '<a class="dismiss-notice" href="' . add_query_arg( 'tgmpa-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . __( '不再提醒', $this->domain ) . '</a>',
						)
					);
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function _gather_plugin_data() {
			...
			foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
				...
				if ( isset( $plugin['external_url'] ) ) {
					...
					$table_data[$i]['source'] = __( '第三方', TGM_Plugin_Activation::$instance->domain );
				}
				elseif ( isset( $plugin['source'] ) ) {
					/** The plugin must be from a private repository */
					if ( preg_match( '|^http(s)?://|', $plugin['source'] ) )
						$table_data[$i]['source'] = __( '主题扩展', TGM_Plugin_Activation::$instance->domain );
					/** The plugin is pre-packaged with the theme */
					else
						$table_data[$i]['source'] = __( '压缩包', TGM_Plugin_Activation::$instance->domain );
				}
				/** The plugin is from the WordPress repository */
				else {
					$table_data[$i]['source'] = __( '插件库', TGM_Plugin_Activation::$instance->domain );
				}
				...
				elseif ( is_plugin_inactive( $plugin['file_path'] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', __( '未激活', TGM_Plugin_Activation::$instance->domain ) );
		...
		public function no_items() {
			...
			printf( __( '无需任何安装或激活操作。<a href="%1$s" title="返回仪表盘">返回仪表盘</a>', TGM_Plugin_Activation::$instance->domain ), admin_url() );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		...
		public function get_columns() {
			...
			$columns = array(
				...
				'plugin' => __( '名称', TGM_Plugin_Activation::$instance->domain ),
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				$imploded    = empty( $plugin_names ) ? '<strong>' . $last_plugin . '</strong>' : '<strong>' . ( implode( ', ', $plugin_names ) . '</strong> 和 <strong>' . $last_plugin . '</strong>.' );
				...
				if ( is_wp_error( $activate ) )
					...
					printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( '以下插件激活成功： ', '以下插件激活成功： ', $count, TGM_Plugin_Activation::$instance->domain ), $imploded );
	...
	if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
		...
		class TGM_Bulk_Installer extends WP_Upgrader {
			...
			public function install_strings() {
				...
				$this->strings['no_package']          = __( '未找到安装源。', TGM_Plugin_Activation::$instance->domain );
				$this->strings['downloading_package'] = __( '正在下载<span class="code">%s</span>&#8230;', TGM_Plugin_Activation::$instance->domain );
				$this->strings['unpack_package']      = __( '正在解压&#8230;', TGM_Plugin_Activation::$instance->domain );
				$this->strings['installing_package']  = __( '正在安装&#8230;', TGM_Plugin_Activation::$instance->domain );
				$this->strings['process_failed']      = __( '安装失败！', TGM_Plugin_Activation::$instance->domain );
				$this->strings['process_success']     = __( '安装成功！', TGM_Plugin_Activation::$instance->domain );
			...
			public function activate_strings() {
				...
				$this->strings['activation_failed']  = __( '激活失败！', TGM_Plugin_Activation::$instance->domain );
				$this->strings['activation_success'] = __( '激活成功！', TGM_Plugin_Activation::$instance->domain );
	...
	if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
		...
		class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
			...
			public function add_strings() {
				...
				if ( TGM_Plugin_Activation::$instance->is_automatic ) {
					$this->upgrader->strings['skin_upgrade_start']        = __( '正在安装并激活，请耐心等待...', TGM_Plugin_Activation::$instance->domain );
					$this->upgrader->strings['skin_update_successful']    = __( '%1$s 安装并激活成功！', TGM_Plugin_Activation::$instance->domain ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( '详情', TGM_Plugin_Activation::$instance->domain ) . '</span><span class="hidden">' . __( '隐藏', TGM_Plugin_Activation::$instance->domain ) . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = __( '操作完成！', TGM_Plugin_Activation::$instance->domain );
					$this->upgrader->strings['skin_before_update_header'] = __( '正在安装并激活 %1$s (%2$d/%3$d)', TGM_Plugin_Activation::$instance->domain );
				}
				/** Default installation strings */
				else {
					$this->upgrader->strings['skin_upgrade_start']        = __( '正在安装并激活，请耐心等待...', TGM_Plugin_Activation::$instance->domain );
					$this->upgrader->strings['skin_update_failed_error']  = __( '%1$s 安装失败：<strong>%2$s</strong>.', TGM_Plugin_Activation::$instance->domain );
					$this->upgrader->strings['skin_update_failed']        = __( '%1$s 安装失败！', TGM_Plugin_Activation::$instance->domain );
					$this->upgrader->strings['skin_update_successful']    = __( '%1$s 安装成功！', TGM_Plugin_Activation::$instance->domain ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( '详情', TGM_Plugin_Activation::$instance->domain ) . '</span><span class="hidden">' . __( '隐藏', TGM_Plugin_Activation::$instance->domain ) . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = __( '操作完成！', TGM_Plugin_Activation::$instance->domain );
					$this->upgrader->strings['skin_before_update_header'] = __( '正在安装 %1$s (%2$d/%3$d)', TGM_Plugin_Activation::$instance->domain );
				}
			...
			public function bulk_footer() {
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . __( '返回仪表盘', TGM_Plugin_Activation::$instance->domain ) . '">' . __( '返回仪表盘', TGM_Plugin_Activation::$instance->domain ) . '</a>' ) . '</p>';
					echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
				}

...
<ul class="clearfix">
	...
	<?php elseif ( $wp_query->max_num_pages > 1 && ( is_home() || is_archive() || is_search() ) ) : // navigation links for home, archive, and search pages ?>
		...
		<li class="nav-previous"><?php previous_posts_link( __( '<i class="fa fa-chevron-left"></i>', 'framework' ) ); ?></li>
		...
		<li class="nav-next"><?php next_posts_link( __( '<i class="fa fa-chevron-right"></i>', 'framework' ) ); ?></li>
...
function ht_entry_date_legacy() {
	...
	if ( $post_date > $month_ago ) {
		...
		$post_date = sprintf( __( '%1$s前', 'framework' ), human_time_diff( get_the_time('U'), current_time('timestamp', 1) ) );
...
function ht_human_time_diff($from, $to){
	...
	if( $diff >= 0 && $diff < 10 ){
		...
		return sprintf( __("%s前", "framework"), human_time_diff($from, $to));
...
function ht_entry_date() {
	...
	$post_date =  ht_human_time_diff( get_the_time('U'), current_time('timestamp', 1) ) ;

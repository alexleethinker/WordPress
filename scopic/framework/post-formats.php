...
function ht_post_format_standard() {
	...
	<a class="readmore" href="<?php the_permalink(); ?>" rel="nofollow"><?php _e('查看详情','framework') ?></a>
    </div>
	</figcaption>
    <?php } ?>
...
function ht_post_format_audio() {
	...
	if ( $ht_pf_audio_oembed ) {
		
	echo wp_oembed_get($ht_pf_audio_oembed, array('width'=>800)); 
	
	} else {
		
	// Get attachment URLs
	$ht_pf_audio_file_url = wp_get_attachment_url( $ht_pf_audio_file );
	
		echo do_shortcode( '[audio src="'. $ht_pf_audio_file_url .'" width="800" height="100%"]' );
...
function ht_post_format_gallery() {
	...
	// Setup counter
	$i = 0;
	// Don't allow more than 5 if the array is bigger than 6
	if ( !is_single() ) { $maxiterations = 5; } else { $maxiterations = 9999; }
	...
	<figcaption class="entry-thumb-caption">
	<div class="caption-content">
	<a class="caption-link" href="<?php echo $pf_gallery_image_full['url']; ?>" rel="nofollow" title="<?php echo $pf_gallery_image['caption']; ?>"><i class="fa fa-search"></i></a>
...
function ht_post_format_image() {
	...
	<?php echo wp_oembed_get($ht_pf_image_oembed, array('width'=>600)); ?>
	</figure>
...
function ht_post_format_video() {
	...
	<?php 
	if ( $ht_pf_video_oembed ) {
		
		if ( is_single() ) {
		echo wp_oembed_get($ht_pf_video_oembed, array('width'=>658));
		} else {
		echo wp_oembed_get($ht_pf_video_oembed, array('width'=>450));	
		}
	
	} else {
		
	// Get attachment URLs
	$ht_pf_video_file_url = wp_get_attachment_url( $ht_pf_video_file );
	$ht_pf_video_file_poster_url = wp_get_attachment_url( $ht_pf_video_file_poster );
	
	if ( $ht_pf_video_file_poster_url != null ) {
		echo do_shortcode( '[video src="'. $ht_pf_video_file_url .'" width="658" height="100%" poster="'. $ht_pf_video_file_poster_url .'"]' );
	} else {
		echo do_shortcode( '[video src="'. $ht_pf_video_file_url .'" width="658" height="100%"]' );	
	}

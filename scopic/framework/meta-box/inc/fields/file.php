	...
	class RWMB_File_Field extends RWMB_Field
	{
		...
		static function admin_enqueue_scripts()
		{
			...
			wp_localize_script( 'rwmb-file', 'rwmbFile', array(
				'maxFileUploadsSingle' => __( '上传文件超过 %d 大小', 'rwmb' ),
				'maxFileUploadsPlural' => __( '上传文件超过 %d 大小', 'rwmb' ),
			) );
		}
		...
		static function wp_ajax_delete_file()
		{
			...
			if ( $ok )
				...
				wp_send_json_error( __( '错误：无法删除文件。', 'rwmb' ) );
		}
		...
		{
			$i18n_delete = apply_filters( 'rwmb_file_delete_string', _x( '删除', 'file upload', 'rwmb' ) );
			$i18n_edit   = apply_filters( 'rwmb_file_edit_string', _x( '编辑', 'file upload', 'rwmb' ) );
			$li = '
			<li>
